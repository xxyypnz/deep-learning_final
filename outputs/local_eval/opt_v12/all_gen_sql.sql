...
DROP AGGREGATE IF EXISTS agg122_final(text);
DROP FUNCTION IF EXISTS agg122_sfunc(text, text);
DROP FUNCTION IF EXISTS agg122_finalfn(text);
CREATE FUNCTION agg122_sfunc(state text, val text) RETURNS text LANGUAGE sql AS $$
  SELECT coalesce(state, '') || coalesce(val, '')
$$;
CREATE FUNCTION agg122_finalfn(state text) RETURNS text LANGUAGE sql AS $$
  SELECT coalesce(state, '<null>')
$$;
CREATE AGGREGATE agg122_final(text) (
  SFUNC = agg122_sfunc,
  STYPE = text,
  FINALFUNC = agg122_finalfn
);
SELECT agg122_final(v) FROM (VALUES ('a'), ('b'), (NULL)) s(v);
SELECT agg122_final(v) FROM (SELECT NULL::text AS v WHERE false) s;
DROP AGGREGATE IF EXISTS agg122_final(text);
DROP FUNCTION IF EXISTS agg122_sfunc(text, text);
DROP FUNCTION IF EXISTS agg122_finalfn(text);
...
DROP TABLE IF EXISTS hash129_big_a CASCADE;
DROP TABLE IF EXISTS hash129_big_b CASCADE;
SET work_mem = '64kB';
SET enable_mergejoin = off;
SET enable_nestloop = off;
SET enable_hashjoin = on;
CREATE TABLE hash129_big_a (id int, payload text);
CREATE TABLE hash129_big_b (id int, payload text);
INSERT INTO hash129_big_a SELECT g, repeat('a', 800000) FROM generate_series(1, 4) g;
INSERT INTO hash129_big_b SELECT g, repeat('b', 800000) FROM generate_series(1, 4) g;
EXPLAIN (ANALYZE, COSTS OFF, TIMING OFF, SUMMARY OFF)
SELECT count(b.payload) FROM hash129_big_a a JOIN hash129_big_b b ON a.id = b.id;
RESET enable_mergejoin;
RESET enable_nestloop;
RESET enable_hashjoin;
RESET work_mem;
