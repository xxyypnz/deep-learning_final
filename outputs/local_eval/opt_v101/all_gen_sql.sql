...
DROP VIEW IF EXISTS rte_alias_mix101_v CASCADE;
CREATE VIEW rte_alias_mix101_v AS
WITH cte101(x) AS (VALUES (4))
SELECT gs.generate_series AS g, sq.a AS s, vv.b AS v, cte101.x AS c
FROM generate_series(1,2),
     (SELECT 10 AS a) sq,
     (VALUES (20)) vv(b),
     cte101;
SELECT pg_get_viewdef('rte_alias_mix101_v'::regclass, true);
DROP VIEW IF EXISTS rte_alias_mix101_v CASCADE;

-- ===== Test Case 14 (commit 101) =====
DROP VIEW IF EXISTS rte_rowsfrom101_v CASCADE;
CREATE VIEW rte_rowsfrom101_v AS
SELECT rf.generate_series, rf.generate_series_1
FROM ROWS FROM (generate_series(1,2), generate_series(3,4)) rf;
SELECT pg_get_viewdef('rte_rowsfrom101_v'::regclass, true);
DROP VIEW IF EXISTS rte_rowsfrom101_v CASCADE;

-- ===== Test Case 15 (commit 101) =====
DROP VIEW IF EXISTS rte_update_from101_v CASCADE;
DROP TABLE IF EXISTS rte_update_from101_t CASCADE;
CREATE TABLE rte_update_from101_t (id int, val int);
CREATE VIEW rte_update_from101_v AS SELECT * FROM rte_update_from101_t;
CREATE RULE rte_update_from101_r AS ON UPDATE TO rte_update_from101_v DO INSTEAD
  UPDATE rte_update_from101_t SET val = sq.x
  FROM (SELECT 1 AS x) sq, (VALUES (1)) vv(y)
  WHERE rte_update_from101_t.id = vv.y;
SELECT pg_get_ruledef(oid, true) FROM pg_rewrite WHERE ev_class='rte_update_from101_v'::regclass AND rulename='rte_update_from101_r';
DROP VIEW IF EXISTS rte_update_from101_v CASCADE;
DROP TABLE IF EXISTS rte_update_from101_t CASCADE;
...
-- ===== Test Case 37 (commit 110) =====
SELECT 1;

-- ===== Test Case 38 (commit 111) =====
-- Setup
...
DROP VIEW IF EXISTS rw_null120_v CASCADE;
DROP TABLE IF EXISTS rw_null120_log CASCADE;
DROP TABLE IF EXISTS rw_null120_base CASCADE;
CREATE TABLE rw_null120_base (a int, b int);
CREATE TABLE rw_null120_log (a int, b int);
CREATE VIEW rw_null120_v AS SELECT a,b FROM rw_null120_base;
CREATE RULE rw_null120_rule AS ON INSERT TO rw_null120_v DO ALSO
  INSERT INTO rw_null120_log VALUES (DEFAULT, DEFAULT), (DEFAULT, NEW.b), (NEW.a, DEFAULT);
INSERT INTO rw_null120_v VALUES (DEFAULT, 1), (2, DEFAULT), (DEFAULT, DEFAULT);
SELECT * FROM rw_null120_base ORDER BY a NULLS FIRST, b NULLS FIRST;
SELECT * FROM rw_null120_log ORDER BY a NULLS FIRST, b NULLS FIRST;
DROP VIEW IF EXISTS rw_null120_v CASCADE;
DROP TABLE IF EXISTS rw_null120_log CASCADE;
DROP TABLE IF EXISTS rw_null120_base CASCADE;

-- ===== Test Case 73 (commit 120) =====
DROP VIEW IF EXISTS rw_select120_v CASCADE;
DROP TABLE IF EXISTS rw_select120_log CASCADE;
DROP TABLE IF EXISTS rw_select120_base CASCADE;
CREATE TABLE rw_select120_base (a int, b int);
CREATE TABLE rw_select120_log (a int, b int);
CREATE VIEW rw_select120_v AS SELECT a,b FROM rw_select120_base;
CREATE RULE rw_select120_rule AS ON INSERT TO rw_select120_v DO ALSO
  INSERT INTO rw_select120_log SELECT * FROM (VALUES (DEFAULT, NEW.b), (NEW.a, DEFAULT), (DEFAULT, DEFAULT)) AS v(a,b);
INSERT INTO rw_select120_v VALUES (DEFAULT, 3), (4, DEFAULT);
SELECT * FROM rw_select120_log ORDER BY a NULLS FIRST, b NULLS FIRST;
DROP VIEW IF EXISTS rw_select120_v CASCADE;
DROP TABLE IF EXISTS rw_select120_log CASCADE;
DROP TABLE IF EXISTS rw_select120_base CASCADE;

-- ===== Test Case 74 (commit 120) =====
DROP VIEW IF EXISTS rw_mixednull120_v CASCADE;
DROP TABLE IF EXISTS rw_mixednull120_log CASCADE;
DROP TABLE IF EXISTS rw_mixednull120_base CASCADE;
CREATE TABLE rw_mixednull120_base (a int, b int);
CREATE TABLE rw_mixednull120_log (a int, b int);
CREATE VIEW rw_mixednull120_v AS SELECT a,b FROM rw_mixednull120_base;
CREATE RULE rw_mixednull120_rule AS ON INSERT TO rw_mixednull120_v DO ALSO
  INSERT INTO rw_mixednull120_log VALUES (9, DEFAULT), (DEFAULT, 10), (11, 12);
INSERT INTO rw_mixednull120_v VALUES (DEFAULT, DEFAULT), (5, DEFAULT);
SELECT * FROM rw_mixednull120_log ORDER BY a NULLS FIRST, b NULLS FIRST;
DROP VIEW IF EXISTS rw_mixednull120_v CASCADE;
DROP TABLE IF EXISTS rw_mixednull120_log CASCADE;
DROP TABLE IF EXISTS rw_mixednull120_base CASCADE;

-- ===== Test Case 75 (commit 121) =====
...
-- ===== Test Case 86 (commit 124) =====
SELECT 1;

-- ===== Test Case 87 (commit 125) =====
-- Setup
...
-- ===== Test Case 109 (commit 131) =====
SELECT 1;

-- ===== Test Case 110 (commit 132) =====
...
-- ===== Test Case 138 (commit 141) =====
SELECT 1;

-- ===== Test Case 139 (commit 142) =====
