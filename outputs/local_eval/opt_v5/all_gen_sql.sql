...
DROP TABLE IF EXISTS rules_src_101 CASCADE;
DROP TABLE IF EXISTS rules_log_101 CASCADE;
CREATE TABLE rules_src_101(f1 int, f2 int);
CREATE TABLE rules_log_101(f1 int, f2 int, tag text DEFAULT 'tag');
CREATE RULE r101_5 AS ON INSERT TO rules_src_101 DO INSTEAD
  INSERT INTO rules_log_101 AS trgt SELECT NEW.*, 'new' RETURNING trgt.f1, trgt.f2;
CREATE RULE r101_6 AS ON UPDATE TO rules_src_101 DO INSTEAD
  UPDATE rules_log_101 AS trgt SET tag = 'updated' WHERE trgt.f1 = new.f1;
SELECT pg_get_ruledef(oid, true) FROM pg_rewrite WHERE ev_class='rules_src_101'::regclass ORDER BY rulename;
DROP TABLE IF EXISTS rules_src_101 CASCADE;
DROP TABLE IF EXISTS rules_log_101 CASCADE;

-- ===== Test Case 10 (commit 101) =====
DROP TABLE IF EXISTS rules_src_101b CASCADE;
DROP TABLE IF EXISTS rules_log_101b CASCADE;
DROP TABLE IF EXISTS int4_tbl_101b CASCADE;
CREATE TABLE rules_src_101b(f1 int, f2 int);
CREATE TABLE rules_log_101b(f1 int, f2 int, tag text DEFAULT 'tag');
CREATE TABLE int4_tbl_101b(f1 int);
INSERT INTO int4_tbl_101b VALUES (0), (1);
CREATE RULE r101_7 AS ON DELETE TO rules_src_101b DO INSTEAD
  WITH wins AS (INSERT INTO int4_tbl_101b AS trgt VALUES (0) RETURNING *),
       wupd AS (UPDATE int4_tbl_101b trgt SET f1 = f1 + 1 RETURNING *),
       wdel AS (DELETE FROM int4_tbl_101b trgt WHERE f1 = 0 RETURNING *)
  INSERT INTO rules_log_101b AS trgt SELECT old.*, 'old' FROM wins, wupd, wdel RETURNING trgt.f1, trgt.f2;
SELECT pg_get_ruledef(oid, true) FROM pg_rewrite WHERE ev_class='rules_src_101b'::regclass AND rulename='r101_7';
DROP TABLE IF EXISTS rules_src_101b CASCADE;
DROP TABLE IF EXISTS rules_log_101b CASCADE;
DROP TABLE IF EXISTS int4_tbl_101b CASCADE;

-- ===== Test Case 11 (commit 102) =====
SELECT 1;

-- ===== Test Case 12 (commit 103) =====
SELECT 1;

-- ===== Test Case 13 (commit 104) =====
...
DROP TABLE IF EXISTS r118_replace CASCADE;
CREATE TABLE r118_replace(id int);
CREATE RULE r118_normal AS ON INSERT TO r118_replace DO ALSO SELECT 1;
CREATE OR REPLACE RULE "_RETURN" AS ON INSERT TO r118_replace DO ALSO SELECT 2;
DROP TABLE IF EXISTS r118_replace CASCADE;

-- ===== Test Case 61 (commit 119) =====
SELECT 1;
...
DROP TABLE IF EXISTS fk127_pk CASCADE;
DROP TABLE IF EXISTS fk127_fk CASCADE;
CREATE TABLE fk127_pk(a int, b int, PRIMARY KEY(a,b));
CREATE TABLE fk127_fk(a int, b int) PARTITION BY LIST(a);
ALTER TABLE fk127_fk ADD FOREIGN KEY (a,b) REFERENCES fk127_pk ON UPDATE CASCADE ON DELETE CASCADE;
CREATE TABLE fk127_fk_1(a int, b int);
ALTER TABLE fk127_fk_1 ADD CONSTRAINT fk127_fk_a_b_fkey CHECK (a IS NULL OR a IS NOT NULL);
ALTER TABLE fk127_fk ATTACH PARTITION fk127_fk_1 FOR VALUES IN (1);
SELECT conname, contype FROM pg_constraint WHERE conrelid='fk127_fk_1'::regclass ORDER BY conname;
DROP TABLE IF EXISTS fk127_fk CASCADE;
DROP TABLE IF EXISTS fk127_pk CASCADE;
...
-- ===== Test Case 102 (commit 131) =====
SELECT 1;

-- ===== Test Case 103 (commit 132) =====
...
-- ===== Test Case 131 (commit 141) =====
SELECT 1;

-- ===== Test Case 132 (commit 142) =====
