...
DROP VIEW IF EXISTS rte_func_named101_v CASCADE;
DROP FUNCTION IF EXISTS rte_func_named101() CASCADE;
CREATE FUNCTION rte_func_named101() RETURNS TABLE(a int, b text) LANGUAGE sql AS $$ SELECT 1, 'x'::text $$;
CREATE VIEW rte_func_named101_v AS SELECT 1 AS marker FROM rte_func_named101();
SELECT pg_get_viewdef('rte_func_named101_v'::regclass, true);
DROP VIEW IF EXISTS rte_func_named101_v CASCADE;
DROP FUNCTION IF EXISTS rte_func_named101() CASCADE;

-- ===== Test Case 14 (commit 101) =====
DROP VIEW IF EXISTS rte_rowsfrom101_v CASCADE;
CREATE VIEW rte_rowsfrom101_v AS SELECT 1 AS marker FROM ROWS FROM (generate_series(1,2), generate_series(3,4));
SELECT pg_get_viewdef('rte_rowsfrom101_v'::regclass, true);
DROP VIEW IF EXISTS rte_rowsfrom101_v CASCADE;

-- ===== Test Case 15 (commit 102) =====
SELECT 1;

-- ===== Test Case 16 (commit 103) =====
SELECT 1;
...
DROP VIEW IF EXISTS rw_plain120_v CASCADE;
DROP TABLE IF EXISTS rw_plain120_log CASCADE;
DROP TABLE IF EXISTS rw_plain120_base CASCADE;
CREATE TABLE rw_plain120_base (a int DEFAULT 10, b int DEFAULT 20);
CREATE TABLE rw_plain120_log (a int, b int);
CREATE VIEW rw_plain120_v AS SELECT a,b FROM rw_plain120_base;
CREATE RULE rw_plain120_rule AS ON INSERT TO rw_plain120_v DO ALSO
  INSERT INTO rw_plain120_log VALUES (DEFAULT, 11), (22, DEFAULT), (33, 44);
INSERT INTO rw_plain120_v VALUES (DEFAULT, DEFAULT), (1, 2);
SELECT * FROM rw_plain120_log ORDER BY a NULLS FIRST, b NULLS FIRST;
DROP VIEW IF EXISTS rw_plain120_v CASCADE;
DROP TABLE IF EXISTS rw_plain120_log CASCADE;
DROP TABLE IF EXISTS rw_plain120_base CASCADE;
...
DROP TABLE IF EXISTS fk127_real_parent CASCADE;
DROP TABLE IF EXISTS fk127_real_child CASCADE;
DROP TABLE IF EXISTS fk127_real_ref CASCADE;
CREATE TABLE fk127_real_ref (id int PRIMARY KEY);
CREATE TABLE fk127_real_parent (id int NOT NULL, ref_id int, CONSTRAINT fk127_real_parent_ref_id_fkey FOREIGN KEY (ref_id) REFERENCES fk127_real_ref(id)) PARTITION BY LIST (id);
CREATE TABLE fk127_real_child (id int NOT NULL, ref_id int);
ALTER TABLE fk127_real_child ADD CONSTRAINT fk127_real_parent_ref_id_fkey FOREIGN KEY (ref_id) REFERENCES fk127_real_ref(id);
ALTER TABLE fk127_real_parent ATTACH PARTITION fk127_real_child FOR VALUES IN (1);
SELECT conname FROM pg_constraint WHERE conrelid='fk127_real_child'::regclass ORDER BY conname;
DROP TABLE IF EXISTS fk127_real_parent CASCADE;
DROP TABLE IF EXISTS fk127_real_child CASCADE;
DROP TABLE IF EXISTS fk127_real_ref CASCADE;
