...
\! /bin/bash -lc 'set -e; BIN=/workspaces/deep-learning_final/postgresql-13.23/install_coverage/bin; D=/tmp/pgcov_child_hash114_opt_v23_data; S=/tmp/pgcov_child_hash114_opt_v23_sock; LOG=/tmp/pgcov_child_hash114_opt_v23.log; rm -rf "$D" "$S"; mkdir -p "$S"; : > "$LOG"; "${BIN}/initdb" -D "$D" >>"$LOG" 2>&1; "${BIN}/pg_ctl" -D "$D" -l "$LOG" -o "-k $S -p 55433" start >>"$LOG" 2>&1; "${BIN}/createdb" -h "$S" -p 55433 regression >>"$LOG" 2>&1; "${BIN}/psql" -h "$S" -p 55433 -d regression -v ON_ERROR_STOP=1 -c "CREATE TABLE hash_replay114 (id int, payload text); CREATE INDEX hash_replay114_idx ON hash_replay114 USING hash (id); INSERT INTO hash_replay114 SELECT g, repeat(md5(g::text), 8) FROM generate_series(1, 20000) g; CHECKPOINT; INSERT INTO hash_replay114 SELECT g, repeat(md5(g::text), 8) FROM generate_series(20001, 60000) g;" >>"$LOG" 2>&1; "${BIN}/pg_ctl" -D "$D" stop -m immediate >>"$LOG" 2>&1; "${BIN}/pg_ctl" -D "$D" -l "$LOG" -o "-k $S -p 55433" start >>"$LOG" 2>&1; "${BIN}/psql" -h "$S" -p 55433 -d regression -v ON_ERROR_STOP=1 -c "SELECT count(*) FROM hash_replay114" >>"$LOG" 2>&1; "${BIN}/pg_ctl" -D "$D" stop -m fast >>"$LOG" 2>&1' >/tmp/pgcov_child_hash114_opt_v23_outer.log 2>&1
...
\! /bin/bash -lc 'set -e; BIN=/workspaces/deep-learning_final/postgresql-13.23/install_coverage/bin; D=/tmp/pgcov_child_heap115_opt_v23_data; S=/tmp/pgcov_child_heap115_opt_v23_sock; LOG=/tmp/pgcov_child_heap115_opt_v23.log; rm -rf "$D" "$S"; mkdir -p "$S"; : > "$LOG"; "${BIN}/initdb" -D "$D" >>"$LOG" 2>&1; echo "wal_log_hints = on" >> "$D/postgresql.conf"; "${BIN}/pg_ctl" -D "$D" -l "$LOG" -o "-k $S -p 55434" start >>"$LOG" 2>&1; "${BIN}/createdb" -h "$S" -p 55434 regression >>"$LOG" 2>&1; "${BIN}/psql" -h "$S" -p 55434 -d regression -v ON_ERROR_STOP=1 -c "CREATE TABLE heap_visible_replay115 (id int primary key, payload text) WITH (autovacuum_enabled=false)" -c "INSERT INTO heap_visible_replay115 SELECT g, repeat(chr(118), 200) FROM generate_series(1, 20000) g" -c "CHECKPOINT" -c "VACUUM heap_visible_replay115" >>"$LOG" 2>&1; "${BIN}/pg_ctl" -D "$D" stop -m immediate >>"$LOG" 2>&1; "${BIN}/pg_ctl" -D "$D" -l "$LOG" -o "-k $S -p 55434" start >>"$LOG" 2>&1; "${BIN}/psql" -h "$S" -p 55434 -d regression -v ON_ERROR_STOP=1 -c "SELECT count(*) FROM heap_visible_replay115" >>"$LOG" 2>&1; "${BIN}/pg_ctl" -D "$D" stop -m fast >>"$LOG" 2>&1' >/tmp/pgcov_child_heap115_opt_v23_outer.log 2>&1
...
CHECKPOINT;
\! /bin/sh -c "echo \"restore_command = '/bin/false'\" >> /workspaces/deep-learning_final/outputs/local_eval/opt_v23/coverage_workspace/data/postgresql.conf; touch /workspaces/deep-learning_final/outputs/local_eval/opt_v23/coverage_workspace/data/recovery.signal; /workspaces/deep-learning_final/postgresql-13.23/install_coverage/bin/pg_ctl -D /workspaces/deep-learning_final/outputs/local_eval/opt_v23/coverage_workspace/data stop -m fast; /workspaces/deep-learning_final/postgresql-13.23/install_coverage/bin/pg_ctl -D /workspaces/deep-learning_final/outputs/local_eval/opt_v23/coverage_workspace/data -l /workspaces/deep-learning_final/outputs/local_eval/opt_v23/coverage_workspace/server.log -o '-k /tmp/pgcov-55432 -p 55432' start" >/tmp/startupxlog_144_recovery_opt_v23.log 2>&1
\connect regression
SELECT pg_is_in_recovery();
SELECT pg_sleep(1.5);
\connect regression
...
-- ===== Test Case 149 (commit 144) =====
CHECKPOINT;
\! /bin/sh -c "echo \"restore_command = '/bin/false'\" >> /workspaces/deep-learning_final/outputs/local_eval/opt_v23/coverage_workspace/data/postgresql.conf; touch /workspaces/deep-learning_final/outputs/local_eval/opt_v23/coverage_workspace/data/standby.signal; /workspaces/deep-learning_final/postgresql-13.23/install_coverage/bin/pg_ctl -D /workspaces/deep-learning_final/outputs/local_eval/opt_v23/coverage_workspace/data stop -m fast; /workspaces/deep-learning_final/postgresql-13.23/install_coverage/bin/pg_ctl -D /workspaces/deep-learning_final/outputs/local_eval/opt_v23/coverage_workspace/data -l /workspaces/deep-learning_final/outputs/local_eval/opt_v23/coverage_workspace/server.log -o '-k /tmp/pgcov-55432 -p 55432' start; /workspaces/deep-learning_final/postgresql-13.23/install_coverage/bin/pg_ctl -D /workspaces/deep-learning_final/outputs/local_eval/opt_v23/coverage_workspace/data promote -w -t 20" >/tmp/startupxlog_144_standby_opt_v23.log 2>&1
\connect regression
SELECT pg_is_in_recovery();
