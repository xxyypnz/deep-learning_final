...
DROP TABLE IF EXISTS fk127_force_parent CASCADE;
DROP TABLE IF EXISTS fk127_force_child CASCADE;
DROP TABLE IF EXISTS fk127_force_ref CASCADE;
CREATE TABLE fk127_force_ref (id int PRIMARY KEY);
CREATE TABLE fk127_force_parent (id int NOT NULL, ref_id int REFERENCES fk127_force_ref(id)) PARTITION BY LIST (id);
CREATE TABLE fk127_force_child (id int NOT NULL, ref_id int, CONSTRAINT fk127_force_parent_ref_id_fkey CHECK (ref_id IS NULL OR ref_id IS NOT NULL));
ALTER TABLE fk127_force_parent ATTACH PARTITION fk127_force_child FOR VALUES IN (1);
SELECT conname, contype FROM pg_constraint WHERE conrelid='fk127_force_child'::regclass ORDER BY conname, contype;
DROP TABLE IF EXISTS fk127_force_parent CASCADE;
DROP TABLE IF EXISTS fk127_force_child CASCADE;
DROP TABLE IF EXISTS fk127_force_ref CASCADE;

-- ===== Test Case 97 (commit 127) =====
DROP TABLE IF EXISTS fk127_copy_parent CASCADE;
DROP TABLE IF EXISTS fk127_copy_child CASCADE;
DROP TABLE IF EXISTS fk127_copy_ref CASCADE;
CREATE TABLE fk127_copy_ref (id int PRIMARY KEY);
CREATE TABLE fk127_copy_parent (id int NOT NULL, ref_id int CONSTRAINT fk127_copy_parent_ref_id_fkey REFERENCES fk127_copy_ref(id)) PARTITION BY LIST (id);
CREATE TABLE fk127_copy_child (id int NOT NULL, ref_id int);
ALTER TABLE fk127_copy_parent ATTACH PARTITION fk127_copy_child FOR VALUES IN (1);
SELECT conname, contype FROM pg_constraint WHERE conrelid='fk127_copy_child'::regclass ORDER BY conname, contype;
DROP TABLE IF EXISTS fk127_copy_parent CASCADE;
DROP TABLE IF EXISTS fk127_copy_child CASCADE;
DROP TABLE IF EXISTS fk127_copy_ref CASCADE;
